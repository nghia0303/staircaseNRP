...
class NRP:
    def __init__(self, horizon: int, constraint: int=1):
        ...
        self.model = CpoModel()
        self.x = [self.model.binary_var(name=f"x_{i}") for i in range(horizon)]
    ...
    def get_variables_of_day(self, day: int):
        ...
        return self.x[day - 1]  # Adjust for 0-based indexing in Python
    ...
    def add_among_working_day_calendar_week(self, x: int, y: int):
        ...
        for week in range(number_of_weeks):
            ...
            self.model.add(self.model.sum(week_days) >= x)
            self.model.add(self.model.sum(week_days) <= y)
            #
    ...
    def add_at_most_x_working_day_per_y_day(self, x: int, y: int):
        ...
        for start in range(self.horizon - y + 1):
            ...
            self.model.add(self.model.sum(self.x[start:end]) <= x)
    ...
    def solns(self):
        ...
        """
        sol_count = 0
        sol_iter = self.model.start_search(LogVerbosity='Quiet', SearchType='Auto')
