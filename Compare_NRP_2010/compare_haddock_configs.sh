...
for C in "${C_LIST[@]}"; do
  for H in "${H_LIST[@]}"; do
    ...
    #    OUTPUT=$(timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1)
        mem_file=$(mktemp)
        OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r -i10 -c"$C" -h"$H" -n"$N" -na1 -d"$D" -ca1 -a0 -e0 -p0 -t3 -maxP1 -minP1 -wP0 -j1)
        ...
        echo "MDD,64,1,$N,$D,$C,$H,$TIME,$TIME_TO_FIRST_SOL,$PEAK_RAM,$SOLNS" >> "$RESULT_CSV"

        echo "============================================"
    #    OUTPUT=$(timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1)
        mem_file=$(mktemp)
        OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r -i10 -c"$C" -h"$H" -n"$N" -na1 -d"$D" -ca1 -a1 -e0 -p0 -t3 -maxP3 -minP2 -wP1 -j1)
        PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
        TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
        TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
        SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
