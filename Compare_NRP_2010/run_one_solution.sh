...
TIMEOUT=900  # Thời gian timeout cho mỗi lệnh (300 giây)

DATE_STR=$(date +%Y%m%d_%H%M%S)
RESULT_CSV="${SRC_PATH}/Compare_NRP_2010/CSV/result_one_solution_${DATE_STR}.csv"
...
echo "method,width,mode,C,H,totalTime(ms),timeToFirstSol,peak_ram(kb),Sols" > "$RESULT_CSV"
...
#    OUTPUT=$("$AMONG_NURSE_SCRIPT" -w64 -m0 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1)
    cleanup_memory
    mem_file=$(mktemp)
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m0 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1 -so1)
...
#    OUTPUT=$(timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1)
    mem_file=$(mktemp)
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r5 -i10 -c"$C" -h"$H" -n3 -na1 -d3 -ca1 -a1 -e1 -p0 -t3 -maxP1 -minP1 -wP0 -j1 -so1)
    PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
    TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
    TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
    SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
...
#    OUTPUT=$(timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m1 -r5 -i10 -c"$C" -h"$H" -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1)
    mem_file=$(mktemp)
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" "$AMONG_NURSE_SCRIPT" -w64 -m2 -r5 -i10 -c"$C" -h"$H" -n3 -na1 -d3 -ca1 -a1 -e1 -p0 -t3 -maxP1 -minP1 -wP0 -j1 -so1)
    PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
    TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
    TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
    SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
    ...
    for SOLVER in "${SOLVER_LIST[@]}"; do
      cleanup_memory
      echo "Solver: $SOLVER"
      OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" python3 "$STAIRCASE_PATH" "$H" "$C" "staircase-binomial" "$SOLVER" "false" "false" "0" "true")
      PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
      TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
      TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
      SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
    ...
    for SOLVER in "${SOLVER_LIST[@]}"; do
      cleanup_memory
      echo "Solver: $SOLVER"
      OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" python3 "$STAIRCASE_PATH" "$H" "$C" "pblib_bdd-binomial" "$SOLVER" "false" "false" "0" "true")
      PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
      TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
      TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
      SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
    ...
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" python3 "$CPLEX_CP_PATH" "$H" "$C" "true")
    PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
    TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
    TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
    SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
    ...
    echo "============================================"
    echo "CPLEX MP"
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" python3 "$CPLEX_MP_PATH" "$H" "$C" "true")
    PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
    TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
    TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
    SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
    ...
    echo "============================================"
    echo "Gurobi"
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" python3 "$GUROBI_PATH" "$H" "$C" "true")
    ...
    echo "Picat"
    START_TIME=$(date +%s%N)
    OUTPUT=$(/usr/bin/time -f "%M" -o "$mem_file" timeout "$TIMEOUT" picat "$PICAT_PATH" "$C" "$H" "1")
    PEAK_RAM=$(cat "$mem_file"); rm -f "$mem_file"
    END_TIME=$(date +%s%N)
