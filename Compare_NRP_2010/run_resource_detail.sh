...
#    echo "${C} ${H}"
    ...
    bash -c "timeout 300 $AMONG_NURSE_SCRIPT -w64 -m0 -r5 -i10 -c$C -h$H -n0 -na1 -d2 -ca1 -a1 -e1 -p0 -t3 -maxP0 -minP0 -wP0 -j1" | tee >(cat > output.txt) &
    pid=$!

    while kill -0 "$pid" 2> /dev/null; do
        ps -p "$pid" -o pid,%cpu,%mem,rss,nlwp,cmd >> usage_log.txt
        sleep 1
    done

    # Đọc lại kết quả nếu cần
...
#    OUTPUT=$(timeout 300 python3 "$STAIRCASE_PATH" "$H" "$C" "staircase" "mgh")
#    TIME=$(echo "$OUTPUT" | grep '"time" :' | awk '{print $3}' | tr -d ',')
#    TIME_TO_FIRST_SOL=$(echo "$OUTPUT" | grep 'timeToFirstSol' | awk '{print $3}' | tr -d ',')
#    SOLNS=$(echo "$OUTPUT" | grep 'solns' | awk '{print $3}' | tr -d ',')
#
