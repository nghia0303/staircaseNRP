...
void buildModel(CPSolver::Ptr cp, int relaxSize, int mode)
{
   ...
   // These are the constraints for C-I only.
   int L1 = 0;
   int U1 = 6;
   int N1 = 8;
   ...
   if (mode == 0) {
      ...
      for (int i=0; i<H-N1+1; i++) {
         ...
         mdd->post(Factory::amongMDD2(adv, L1, U1, workDay));
      }
      ...
      for (int i=0; i<H-N2+1; i++) {
         ...
         auto adv = all(cp, amongVars, [&vars](int i) {return vars[i];});
         mdd->post(Factory::amongMDD2(adv, L2, U2, workDay));
      }
      ...
      for (int i=0; i<H/N3; i++) {
         ...
         if (7*i+7<H) {
            ...
            auto adv = all(cp, amongVars, [&vars](int i) {return vars[i];});
            mdd->post(Factory::amongMDD2(adv, L3, U3, workDay));
         }
      }
      cp->post(mdd);
   }
      ...
      mdd->post(seqMDD2(vars, N1, L1, U1, workDay));

      // constraint 2
      cout << "Sequence(vars," << N2 << "," << L2 << "," << U2 << ",{1})" << std::endl;
      mdd->post(seqMDD2(vars, N2, L2, U2, workDay));
      ...
      // constraint 1
      cout << "Sequence(vars," << N1 << "," << L1 << "," << U1 << ",{1})" << std::endl;
      mdd->post(Factory::seqMDD3(vars, N1, L1, U1, workDay));

      // constraint 2
      cout << "Sequence(vars," << N2 << "," << L2 << "," << U2 << ",{1})" << std::endl;
      mdd->post(Factory::seqMDD3(vars, N2, L2, U2, workDay));
